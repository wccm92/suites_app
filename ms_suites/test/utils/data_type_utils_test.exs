defmodule MsSandbox.Utils.DataTypeUtilsTest do
  ...
  @valid_hex32_low "a69c1a7835abe4710968ce7be511f3f7e"
  ...
  describe "validate_format?/2 :hex32" do
    test "accepts 32-char uppercase hex" do
      assert DataTypeUtils.validate_format?(@valid_hex32_up, :hex32)
    end

    test "accepts 32-char lowercase hex" do
      assert DataTypeUtils.validate_format?(@valid_hex32_low, :hex32)
    end

    test "rejects wrong length (31/33)" do
      assert {:error, :not_a_string} =
               DataTypeUtils.validate_format?(String.slice(@valid_hex32_up, 0, 31), :hex32)

      assert {:error, :not_a_string} =
               DataTypeUtils.validate_format?(@valid_hex32_up <> "0", :hex32)
    end

    test "rejects non-hex characters" do
      assert {:error, :not_a_string} =
               DataTypeUtils.validate_format?("Z69C1A7835ABE4710968CE7BE511F3F7E", :hex32)

      assert {:error, :not_a_string} =
               DataTypeUtils.validate_format?("A69C1A7835ABE4710968CE7BE511F3F7G", :hex32)
    end

    test "rejects nil and empty" do
      assert {:error, :not_a_string} = DataTypeUtils.validate_format?(nil, :hex32)
      assert {:error, :not_a_string} = DataTypeUtils.validate_format?("", :hex32)
    end

    test "rejects non-binary inputs (integer/atom)" do
      assert {:error, :not_a_string} = DataTypeUtils.validate_format?(123, :hex32)
      assert {:error, :not_a_string} = DataTypeUtils.validate_format?(:abc, :hex32)
    end
  end
end
